...
export function RegistrationSummary({ registration, onDownload, onRestart, onBack }: Props) {
  ...
  const totalCost = React.useMemo(() => {
    const monthlyAccommodation = Math.round(
      registration.accommodation.basePrice / registration.roommateCount
    );
    return registration.program.tuitionFee + monthlyAccommodation * 10; // 10 months
  }, [registration]);

  return (
    <div className="max-w-4xl mx-auto">
      <div className="bg-white shadow-2xl rounded-2xl overflow-hidden border border-gray-100">
        <div className="px-8 py-6 bg-gradient-to-r from-indigo-500 to-indigo-600 border-b border-indigo-600">
            ...
            <dl className="grid grid-cols-1 gap-6">
              ...
              <div className="py-4 sm:grid sm:grid-cols-3 sm:gap-4">
                ...
                <dd className="mt-1 text-xl font-bold text-emerald-600 sm:col-span-2 sm:mt-0">
                  €{totalCost.toLocaleString()} ({t.summary.academicYear})
                </dd>
              </div>
            </dl>
          </div>
